...
static enum parse_result advance_parse_state(struct external_server_msg_s *msg, enum parse_state new_state)
{
...
static enum parse_result parse_op(struct external_server_msg_s *msg, size_t* consumed, size_t* required)
{
    *consumed = 0;
    *required = 0;
...
static enum parse_result parse_payload_len(struct external_server_msg_s *msg, size_t* consumed, size_t* required)
{
    *consumed = 0;
    *required = 0;
...
static enum parse_result parse_payload(struct external_server_msg_s *msg, size_t* consumed, size_t* required)
{
...
typedef enum parse_result (*parse_func_t)(struct external_server_msg_s *, size_t*, size_t*);

static parse_func_t parse_functions[ps_NUM] = {parse_op, parse_payload_len, parse_payload};

static bool parse_loop(struct external_server_msg_s *msg, const char *data, size_t data_size, size_t* data_consumed, size_t *min_required_data)
{
...
{
    memset(msg, 0, sizeof(struct external_server_msg_s));
